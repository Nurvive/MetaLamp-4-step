...
class ViewHead {
    constructor(parent, direction, value, bubbleValue){
        ...
        this.bubble = new _headBubbleDefault.default().element;
        this.template = this.direction === 'horizontal' ? `<div class='slider__head'>
            </div>` : `<div class='slider__head slider__head_vertical'>
            </div>`;
        this.parent.insertAdjacentHTML('afterbegin', this.template); // Здесь приведение через "as" оправдано, так как элемент точно создается строчкой выше
...
class Scale {
    constructor(parent, direction, min, max){
        this.parent = parent;
        this.direction = direction;
        this.template = this.direction === 'horizontal' ? '<div class="slider__scale"></div>' : '<div class="slider__scale slider__scale_vertical"></div>';
        this.parent.insertAdjacentHTML('beforeend', this.template); // Здесь приведение через "as" оправдано, так как элемент точно создается строчкой выше
    ...
    init(min, max) {
        ...
        for(let i = 0; i <= 100; i += 5){
            if (i % 25 === 0) {
                ...
                i === 0 ? number.innerHTML = String(min) : number.innerHTML = String(min + i / 25 * step);
